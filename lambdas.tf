...
resource "aws_lambda_function" "update_source_lambda" {
  function_name = "${var.project}-${var.environment}-update-source"
  runtime       = "java21"
  handler       = "com.example.Handler::handleRequest"
  role          = aws_iam_role.update_sources_role.arn
...
#   s3_key    = var.update_sources_package_key
  ...
  environment {
    variables = {
      SOURCE_QUEUE_URL  = aws_sqs_queue.update_source_queue.id
